-- Migration to remove client role from the system
-- This removes 'client' from the user_role enum and updates any existing client users

-- First, update any existing users with 'client' role to 'worker' role
UPDATE public.users SET role = 'worker' WHERE role = 'client';

-- Update any pending invitations with 'client' role to 'worker' role
UPDATE public.invitations SET role = 'worker' WHERE role = 'client';

-- Drop all functions that depend on user_role enum
DROP FUNCTION IF EXISTS create_invitation(TEXT, user_role, UUID, TEXT);
DROP FUNCTION IF EXISTS create_invitation(TEXT, user_role, UUID, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS accept_invitation(TEXT, UUID);

-- Create a new enum without 'client'
CREATE TYPE user_role_new AS ENUM ('admin', 'sales', 'worker');

-- Drop the default constraints first
...
ALTER TABLE public.invitations ALTER COLUMN role TYPE user_role_new USING role::text::user_role_new;

-- Re-add the default constraint
ALTER TABLE public.users ALTER COLUMN role SET DEFAULT 'worker'::user_role_new;

-- Drop the old enum and rename the new one
DROP TYPE user_role;
ALTER TYPE user_role_new RENAME TO user_role;

-- Recreate the functions with the new enum type and simplified logic
